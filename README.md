...
`app/index.html` をブラウザで開くだけです（サーバー不要・オフライン動作）。
...
```bash
python3 scripts/extract_exams.py   # *定期試験問題.html -> app/exams.json
python3 scripts/build_data.py      # exams.json + flashcards.json -> app/data.js
```

- `scripts/extract_exams.py` — 過去問HTMLを問題単位に構造化し、図表（データURI画像）を保持したままJSON化。キーワードによるテーマ自動分類も行う。
- `app/flashcards.json` — 一問一答の参考解答（手作業で作成）。
- `app/data.js` — `index.html` が `file://` でも読めるよう、上記JSONをJSにまとめたもの。
